class Obstacle { // class tạo ra các chướng ngại vật
    private frameX :number;
    private frameY :number;
    private randomise :number;
    ...
    draw(){
        ...
        if(this.type === 'turtle'){
            ...
            ctx1?.drawImage(turtle, this.frameX * 70, 210, 70, 70, this.x, this.y, this.width, this.height);
        } else if(this.type === 'log'){
            ctx1?.drawImage(log, this.x, this.y, this.width, this.height);
        }else{
            ctx2?.drawImage(car,this.frameX * 300 , this.carType * 230, 270, 200 ,this.x, this.y - 12, this.width, this.height);
        }
...
function handleObstacle(){
    ...
    for(let i = 0; i < carsArray.length; i++){
        if(collision(frogger, carsArray[i])){
            // console.log('die');
            ctx4?.drawImage(collisions, 0, 100, 100, 100, frogger.x - 23, frogger.y - 30, 100, 100);
            resetGame();
        }
    }
    ...
    if(frogger.y > 100 && frogger.y < 250){
        ...
        if(!safe) {
            ...
            ctx4?.drawImage(collisions, 0, 0, 100, 100, frogger.x - 23, frogger.y - 30, 100, 100);
            resetGame();
        }
    }
}
